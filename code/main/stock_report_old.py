...
# 取得加權指數日報
def taiex_daily_report(anaMonths = 4, forceRerun = True):
    stockId = "TAIEX"
    eDt = today
...
def export(stockId, sDt = None, eDt = today, forceRerun = False) -> pd.DataFrame:
    # === 日期設定 ===
    if sDt is None:
        sDt = eDt - relativedelta(months=4)   
    end_plus1 = (eDt + relativedelta(days=1)).strftime("%Y-%m-%d")
    ...
    df = df[df["date"] <= pd.Timestamp(eDt.date())]
    ...
    # === 實體(%)、上影(%)、下影(%) ===
    new_df["實體(%)"] = abs(new_df["收盤價"] - new_df["開盤價"]) / (new_df["最高價"] - new_df["最低價"])
    new_df["上影(%)"] = (new_df["最高價"] - np.maximum(new_df["開盤價"], new_df["收盤價"])) / (
        new_df["最高價"] - new_df["最低價"])
    new_df["下影(%)"] = (np.minimum(new_df["開盤價"], new_df["收盤價"]) - new_df["最低價"]) / (
        new_df["最高價"] - new_df["最低價"])
    ...
    new_df["K線型態"] = new_df.apply(classify_k_type, axis=1)
...
if __name__ == "__main__":
    ...
    # df = export("TAIEX", sDt, eDt, False)
    # # 日期,股票代號,開盤價,收盤價,收盤-開盤,最高價,最低價,日振幅,漲跌幅(%),成交量,量增率(%),5日均量,5日最大量_日期,5日最大量,10日均量,10日最大量_日期,10日最大量,20日均量,20日最大量_日期,20日最大量,實體(%),上影(%),下影(%),K線型態,跳空缺口,5日平均,10日平均,20日平均,5日上升幅度,10日上升幅度,20日上升幅度,5日扣抵值,5日扣抵影響(%),10日扣抵值,10日扣抵影響(%),20日扣抵值,20日扣抵影響(%),均線得分,均線方向,均線排列,均線距離(%),均線狀態,趨勢強度說明,趨勢等級,5日乖離,10日乖離,20日乖離,總成交金額(億),法人總買超(億),買超-外資(億),買超-投信(億),買超-自營商(億),買超-融資(億),資金走向,資金走向判讀

    # new_df = df[["日期", "股票代號", "開盤價", "收盤價", "最高價", "最低價", "成交量", "5日平均"]]
    # new_df.to_csv("tmp.csv", index=False, encoding="utf-8-sig")